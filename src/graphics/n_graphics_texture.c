...
extern const N_Texture* n_graphics_texture_create_from_file(const char *path) {
    ...
    const N_Texture *texture = n_graphics_texture_create((N_Vec4_I32){.x = width, .y = height}, N_TextureFormat_RGBA_F32);
...
extern const N_Texture* n_graphics_texture_create(N_Vec4_I32 size, N_TextureFormat format) {
    ...
    texture->buffer = (N_GraphicsBuffer*)n_graphics_buffer_create(size, n_graphics_texture_format_stride(format));
...
extern N_Vec4_I32 n_graphics_texture_get_size(const N_Texture *texture) {
    return n_graphics_buffer_get_size(texture->buffer);
}
